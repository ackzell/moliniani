...
| `topographic`   | `TopographicBackground`   | Faithful **Canvas-2D** port of the "Topographic Contour Map" sketch (no shader). `color0` color → `#0a0a0a` (backdrop), `color1`–`color3` color (coral → amber → gold ramp), `contours` number → `14` (contour levels — 2–60), `speed` number → `0.15` (field morph speed — 0–1), `noiseScale` number → `0.003` (field frequency, CSS px — 0.001–0.01), `labels` number → `1.0` (elevation-label density — 0–1; 0 hides them), `labelSize` number → `9` (label font size, px — 6–24)                                                                                                                                                                                                                                              |
| `sugarGlass`    | `SugarGlassBackground`    | **GLSL** port of the "Sugar Glass" sketch. `color0` color → `#c8956c`, `color1` color → `#d4a574`, `darkColor` color → `#4a2000` (thick-shard depth), `crackColor` color → `#ffe8c0` (crack bleed), `crackHighlight` color → `#fff5e6` (crack cores), `refractColor` color → `#e6a699` (rose-gold refraction), `crackSpeed` number → `0.5` (fracture animation speed — 0.1–2), `lightBleed` number → `1` (crack light intensity — 0.3–2), `density` number → `1` (fracture-network density — 0.5–3), `crackWidth` number → `1` (crack line width — 0–2)                                                                                                                                                                           |
| `kineticGrid`   | `KineticGridBackground`   | Faithful **Canvas-2D** port of the "Kinetic Grid" sketch (no shader). `lineColor0`–`lineColor4` color (5 warm connection-line stops), `nodeColor0`–`nodeColor2` color (3 cool node stops), `flashColor` color → `#ffd296` (impulse glow/ring/halo), `backdrop` color → `#0a0806`, `impulseRate` number → `0.7` (edge-impulse frequency — 0.3–3), `springTension` number → `1` (mesh spring stiffness — 0.2–3), `impulseForce` number → `1` (impulse kick strength — 0.3–3), `damping` number → `0.978` (per-frame velocity damping — 0.95–0.995), `returnForce` number → `0.003` (pull to rest — 0.001–0.01), `density` number → `1` (scales the 40×25 grid — 0.5–2), `trailFrames` number → `15` (neon trail persistence — 0–40) |
