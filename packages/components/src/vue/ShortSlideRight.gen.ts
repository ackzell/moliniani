...
import { SHORT_SLIDE_RIGHT, resolveEffectKnobs } from "../textEffects";
import { fromState } from "../effectTiming";


const _sfc_main = /*@__PURE__*/_defineComponent({
  __name: 'ShortSlideRight',
...
const split = useSplitUnits(
  ...
  () =>
    ({
      [props.split]: { class: `short-slide-right-${props.split}` },
    }) as TextSplitterParams,
  {
    ...
    unit: () => fromState(resolveEffectKnobs(SHORT_SLIDE_RIGHT, props)),
    ...
    effect: () => ({
      phase: "phase",
      exit: "exit",
      knobs: () => resolveEffectKnobs(SHORT_SLIDE_RIGHT, props),
