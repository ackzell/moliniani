...
import { FADE_THROUGH, resolveEffectKnobs } from "../textEffects";
...
const split = useSplitUnits(
  ...
  () =>
    ({
      [props.split]: { class: `fade-through-${props.split}` },
    }) as TextSplitterParams,
  {
    ...
    unit: () => fromState(resolveEffectKnobs(FADE_THROUGH, props)),
    ...
    effect: () => ({
      phase: "phase",
      exit: "exit",
      knobs: () => resolveEffectKnobs(FADE_THROUGH, props),
