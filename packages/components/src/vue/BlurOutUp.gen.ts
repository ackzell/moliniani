...
import { BLUR_OUT_UP, resolveEffectKnobs } from "../textEffects";
...
const split = useSplitUnits(
  ...
  () =>
    ({
      [props.split]: { class: `blur-out-up-${props.split}` },
    }) as TextSplitterParams,
  {
    ...
    unit: () => fromState(resolveEffectKnobs(BLUR_OUT_UP, props)),
    ...
    effect: () => ({
      phase: "phase",
      exit: "exit",
      knobs: () => resolveEffectKnobs(BLUR_OUT_UP, props),
