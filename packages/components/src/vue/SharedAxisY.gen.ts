...
import { SHARED_AXIS_Y, resolveEffectKnobs } from "../textEffects";
...
const split = useSplitUnits(
  ...
  () =>
    ({
      [props.split]: { class: `shared-axis-y-${props.split}` },
    }) as TextSplitterParams,
  {
    ...
    unit: () => fromState(resolveEffectKnobs(SHARED_AXIS_Y, props)),
    ...
    effect: () => ({
      phase: "phase",
      exit: "exit",
      knobs: () => resolveEffectKnobs(SHARED_AXIS_Y, props),
