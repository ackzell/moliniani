...
import { FOCUS_BLUR_RESOLVE, resolveEffectKnobs } from "../textEffects";
...
const split = useSplitUnits(
  ...
  () =>
    ({
      [props.split]: { class: `focus-blur-resolve-${props.split}` },
    }) as TextSplitterParams,
  {
    ...
    unit: () => fromState(resolveEffectKnobs(FOCUS_BLUR_RESOLVE, props)),
    ...
    effect: () => ({
      phase: "phase",
      exit: "exit",
      knobs: () => resolveEffectKnobs(FOCUS_BLUR_RESOLVE, props),
