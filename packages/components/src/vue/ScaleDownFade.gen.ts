...
import { SCALE_DOWN_FADE, resolveEffectKnobs } from "../textEffects";
import { fromState } from "../effectTiming";


const _sfc_main = /*@__PURE__*/_defineComponent({
  __name: 'ScaleDownFade',
...
const split = useSplitUnits(
  ...
  () =>
    ({
      [props.split]: { class: `scale-down-fade-${props.split}` },
    }) as TextSplitterParams,
  {
    ...
    unit: () => fromState(resolveEffectKnobs(SCALE_DOWN_FADE, props)),
    ...
    effect: () => ({
      phase: "phase",
      exit: "exit",
      knobs: () => resolveEffectKnobs(SCALE_DOWN_FADE, props),
