...
import { STAGGER_FROM_CENTER, resolveEffectKnobs } from "../textEffects";
import { fromState } from "../effectTiming";


const _sfc_main = /*@__PURE__*/_defineComponent({
  __name: 'StaggerFromCenter',
...
const split = useSplitUnits(
  ...
  () =>
    ({
      [props.split]: { class: `stagger-from-center-${props.split}` },
    }) as TextSplitterParams,
  {
    ...
    unit: () => fromState(resolveEffectKnobs(STAGGER_FROM_CENTER, props)),
    ...
    effect: () => ({
      phase: "phase",
      exit: "exit",
      knobs: () => resolveEffectKnobs(STAGGER_FROM_CENTER, props),
