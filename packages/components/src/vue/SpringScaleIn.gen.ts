...
import { buildEffectAnimation, SPRING_SCALE_IN, type TextEffectProps } from "../textEffects";
...
const anime = useSplitTextAnimation(
  el,
  () => {
    return { [props.split]: { class: `spring-scale-in-${props.split}` } } as TextSplitterParams;
  },
  () => buildEffectAnimation(SPRING_SCALE_IN, props as TextEffectProps),
  {
    progress: "progress",
